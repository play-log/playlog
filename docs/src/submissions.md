...
Where `md5(password)` is an MD5 checksum of the user's `password` and `timestamp` is the same
...
These values may change per handshake and should be used for one listening
